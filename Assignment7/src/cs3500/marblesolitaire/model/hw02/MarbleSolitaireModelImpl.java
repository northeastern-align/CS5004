...
public class MarbleSolitaireModelImpl implements MarbleSolitaireModel {
  ...
  private List<IRule> allPossibleCommands = new ArrayList<IRule>();
  private IBoard board;

  /**
   * A constructor that creates a gameboard with a default start space location at 3, 3. This
   * constructor stores the rules to a list of all possible commands.
   */
  ...
  public MarbleSolitaireModelImpl(int row, int col) {
    // Board throws IllegalArgumentException if row/col are out of bounds
    this.board = new Board(row, col);
    allPossibleCommands.add(new HorizontalMove());
    allPossibleCommands.add(new VerticalMove());
  }

  public MarbleSolitaireModelImpl(int width) {
   ...
   *
   * @return a boolean stating whether the game is over (true) or can continue (false)
   */
  @Override
   ...
   * Empty spaces return a space. The string is built iteratively as the double array is progressed
   * through.
   *
